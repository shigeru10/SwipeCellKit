...
protocol SwipeActionsTableViewDelegate: class {
    func swipeActionsTableView(_ swipeActionsTableView: SwipeActionsTableView, didSelect action: SwipeAction)
}

class SwipeActionsTableView: UIView {
    weak var delegate: SwipeActionsTableViewDelegate?
    ...
    var expansionTableDelegate: SwipeExpanding? {
        return options.expansionTableDelegate ?? (expandableAction?.hasBackgroundColor == false ? ScaleAndAlphaExpansion.default : nil)
    }
    ...
    func actionTapped(button: SwipeActionButton) {
        ...
        delegate?.swipeActionsTableView(self, didSelect: actions[index])
    }
    ...
    func setExpanded(expanded: Bool, feedback: Bool = false) {
        ...
        let timingParameters = expansionTableDelegate?.animationTimingParameters(buttons: buttons.reversed(), expanding: expanded)
    ...
    func notifyExpansion(expanded: Bool) {
        ...
        expansionTableDelegate?.actionButton(expandedButton, didChange: expanded, otherActionButtons: buttons.dropLast().reversed())
    }
